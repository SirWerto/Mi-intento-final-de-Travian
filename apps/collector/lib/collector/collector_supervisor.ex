defmodule Collector.CollectorSupervisor do
  ...
  @spec start_worker_info(server_id :: TTypes.server_id()) ::
  ...
  def start_worker_info(server_id) do
    case DynamicSupervisor.start_child(__MODULE__, {Collector.GenWorker, [server_id, :info]}) do
      ...
      :ignore ->
        {:error, {server_id, :info, :ignore}}

      {:error, reason} ->
        {:error, {server_id, :info, reason}}
    end
  end

  @spec start_worker_snapshot(server_id :: TTypes.server_id()) ::
          {:ok, {pid(), reference()}} | {:error, any()}
  def start_worker_snapshot(server_id) do
    case DynamicSupervisor.start_child(__MODULE__, {Collector.GenWorker, [server_id, :snapshot]}) do
      ...
      :ignore ->
        {:error, {server_id, :snapshot, :ignore}}

      {:error, reason} ->
        {:error, {server_id, :snapshot, reason}}
    end
  end
end
