defmodule Collector.GenCollector do
  ...
  @spec collect(server_id :: TTypes.server_id()) :: :ok | {:error, any()}
  def collect(server_id) do
    GenServer.call(__MODULE__, {:collect, server_id})
  end
  ...
  @impl true
  def handle_cast({type_c, server_id, pid}, state = %__MODULE__{active_p: active_p, active: true}) when is_map_key(active_p, pid)
    do
  ...
  @impl true
  def handle_info({:DOWN, _ref, :process, pid, :normal}, state = %__MODULE__{active_p: ap, active: true}) when is_map_key(ap, pid) and map_size(ap) == 1 do
    new_state = state
      |> Map.put(:active_p, %{})
  ...
  def handle_info({:DOWN, _ref, :process, pid, :normal}, state = %__MODULE__{active_p: ap, active: true}) when is_map_key(ap, pid) do
    {{type, _ref, server_id, _counter}, new_ap} = Map.pop!(ap, pid)
    Logger.debug(%{msg: "Collector.GenWorker finished", type_collection: type, server_id: server_id})
  ...
  def handle_info({:DOWN, _ref, :process, pid, reason}, state = %__MODULE__{active_p: ap, active: true}) when is_map_key(ap, pid) do
    {{type, _ref, server_id, counter}, new_ap} = Map.pop!(ap, pid)
    Logger.error(%{msg: "Collector.GenWorker down, tries: #{counter}", type_collection: type, reason: reason, server_id: server_id})
    Logger.info(%{msg: "Collector.GenWorker down, tries: #{counter}", type_collection: type, reason: reason, server_id: server_id})
	...
	Logger.warning(%{msg: "Unable to start Collector.GenWorker", type_collection: type, reason: reason, server_id: server_id})
	new_state = state
	|> Map.put(:active_p, %{})
	|> Map.put(:active, false)
	Enum.each(state.subscriptions, &(send(&1, {:collector_event, :collection_finished})))
  ...
  def handle_info(:collect, state = %__MODULE__{active: false}) do
	...
	Enum.each(errors, &(Logger.warning(%{msg: "Unable to start Collector.GenWorker", reason: elem(&1,1)})))

	ap = for {{:ok, {pid, ref}}, server_id, type_c} <- childs, into: %{}, do: {pid, {type_c, ref, server_id, 0}}
	collection_hour = Application.fetch_env!(:collector, :collection_hour)
	wait_time = Collector.Utils.time_until_collection(collection_hour)
	tref = :erlang.send_after(wait_time, self(), :collect)
  ...
  @spec start_child(:snapshot | :metadata, TTypes.server_id()) :: {:ok, {pid(), reference()}} | {:error, any()}
