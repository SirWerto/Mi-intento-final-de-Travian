defmodule Collector.GenWorker.Metadata do
  ...
  def handle_info(:timeout, state = {server_id, max_tries, max_tries}) do
    Logger.info(%{msg: "Unable to collect metadata", type_collection: :metadata, reason: :max_tries, server_id: server_id})
    {:stop, :normal, state}
  end
  ...
  def etl(root_folder, server_id) do
    with(
      Logger.debug(%{msg: "Collector step 1, fetch metadata", type_collection: :metadata, server_id: server_id}),
      {:step_1, {:ok, metadata}} <- {:step_1, :travianmap.get_info(server_id)},
      metadata = Map.put(metadata, "server_id", server_id),
      Logger.debug(%{msg: "Collector step 2, store metadata", type_collection: :metadata, server_id: server_id}),
      encoded_metadata = Collector.metadata_to_format(metadata),
      {:step_2, :ok} <- {:step_2, Storage.store(root_folder, server_id, @flow_metadata, encoded_metadata)}
    )
      do
      Logger.info(%{msg: "Collector metadata success", type_collection: :metadata, server_id: server_id})
      :ok
      else
	{:step_1, {:error, reason}} ->
    	  Logger.info(%{msg: "Collector unable to fetch metadata", reason: reason, type_collection: :metadata, server_id: server_id})
	{:step_2, {:error, reason}} ->
    	  Logger.warning(%{msg: "Collector unable to store metadata", reason: reason, type_collection: :metadata, server_id: server_id})
    end
  end
