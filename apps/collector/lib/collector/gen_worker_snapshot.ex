defmodule Collector.GenWorker.Snapshot do
  ...
  @spec start_link(server_id :: TTypes.server_id(), max_tries :: pos_integer()) :: GenServer.on_start()
  def start_link(server_id, max_tries \\ @max_tries), do: GenServer.start_link(__MODULE__, [server_id, max_tries])
  ...
  def handle_info(:timeout, state = {server_id, max_tries, max_tries}) do
    Logger.info(%{msg: "Unable to collect snapshot", type_collection: :snapshot, reason: :max_tries, server_id: server_id})
    {:stop, :normal, state}
  end
  ...
  def etl(root_folder, server_id) do
    ...
    with(
      Logger.debug(%{msg: "Collector step 1, fetch snapshot", type_collection: :snapshot, server_id: server_id}),
      {:step_1, {:ok, raw_snapshot}} <- {:step_1, :travianmap.get_map(server_id)},
      Logger.debug(%{msg: "Collector step 2, process snapshot", type_collection: :snapshot, server_id: server_id}),
      ...
      Logger.debug(%{msg: "Collector step 3, store snapshot_rows", type_collection: :snapshot, server_id: server_id}),
      encoded_snapshot = Collector.snapshot_to_format(snapshot_rows),
      {:step_3, :ok} <- {:step_3, Storage.store(root_folder, server_id, @flow_snapshot, encoded_snapshot, today)},
      GenServer.cast(Medusa.GenCollector, {:snapshot_collected, server_id, self()}),
      Logger.debug(%{msg: "Collector step 4, store snapshot_errors if there is", type_collection: :snapshot, server_id: server_id}),
      {:step_4, :ok} <- {:step_4, store_errors(root_folder, server_id, snapshot_errors, today)}
    ) do
      Logger.info(%{msg: "Collector snapshot success", type_collection: :snapshot, server_id: server_id})
      :ok
    else
      {:step_1, {:error, reason}} ->
    	Logger.info(%{msg: "Collector unable to fetch snapshot", reason: reason, type_collection: :snapshot, server_id: server_id})
      {:step_3, {:error, reason}} ->
    	Logger.warning(%{msg: "Collector unable to store snapshot", reason: reason, type_collection: :snapshot, server_id: server_id})
      {:step_4, {:error, reason}} ->
    	Logger.info(%{msg: "Collector unable to store snapshot_errors", reason: reason, type_collection: :snapshot, server_id: server_id})
    end
  end


  @spec store_errors(root_folder :: binary(), server_id :: TTypes.server_id(), snapshot_errors :: [any()], date :: Date.t()) :: :ok | {:error, any()}
  ...
  defp store_errors(root_folder, server_id, snapshot_errors, date) do
    ...
    case Storage.store(root_folder, server_id, @flow_snapshot_errors, encoded_snapshot_errors, date) do
      :ok -> 
