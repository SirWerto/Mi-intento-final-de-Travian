defmodule CollectorTest do
  ...
  test "ignore pid down monitors if there aren't in active_process" do
    ...
    assert(
      {:noreply, input_state} ==
        Collector.GenCollector.handle_info(
          {:DOWN, "some ref", :process, pid, :normal},
          input_state
        )
    )
    ...
    assert(
      {:noreply, input_state} ==
        Collector.GenCollector.handle_info(
          {:DOWN, "some ref", :process, pid, :shutdown},
          input_state
        )
    )
  end
  ...
  test "remove pid from active proccess if a down signal is received with reason :normal" do
    ...
    {:noreply, output_state} =
      Collector.GenCollector.handle_info({:DOWN, ref, :process, pid, :normal}, input_state)
  ...
  test "relaunch task if a down signal is received with reason no normal" do
    ...
    {:noreply, output_state} =
      Collector.GenCollector.handle_info({:DOWN, ref, :process, pid, :test_shutdown}, input_state)
  ...
  test "subscribe add a process to the state and monitors it" do
    ...
    {:reply, :subscribed, output_state} =
      Collector.GenCollector.handle_call(:subscribe, {pid, "some tag"}, input_state)
  ...
  test "if suscribed and a server is collected successful, send {:collected, type, server_id} and removes worker from state" do
    ...
    input_state = %Collector.GenCollector{
      tref: nil,
      active_p: %{pid => value},
      subscriptions: %{self() => "some ref"}
    }

    {:noreply, output_state} =
      Collector.GenCollector.handle_cast({:collected, :snapshot, server_id, pid}, input_state)
  ...
  test "unsubscribe removes a process and demonitors it" do
    ...
    {:reply, :unsubscribed, output_state} =
      Collector.GenCollector.handle_call(:unsubscribe, {pid, "some tag"}, input_state)
