defmodule Medusa.Pipeline.Step1 do
  ...
  defp p(date, enriched_rows = [first | _]) do
    ...
    %__MODULE__{
      ...
      date: date,
      total_population: Enum.map(enriched_rows, fn row -> row[:population] end) |> Enum.sum(),
      n_villages: length(enriched_rows),
      village_pop: (for row <- enriched_rows, into: %{}, do: {row[:village_id], row[:population]}),
      tribes_summary: tribe_summary(enriched_rows),
