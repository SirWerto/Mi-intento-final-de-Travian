...
##################
  ...
  def create_pops() do
    let [
      ...
      case decrease == [] do
	true -> 
	  {Enum.sum(populations), n_village, n_active_village, population_increase, 0, populations, increase}
	false -> 
	  IO.inspect({populations})
	  populations = Enum.zip_reduce([populations, decrease], [], fn [pop, dec], acc -> [pop+dec | acc] end)
	  decrease = Enum.map(decrease, fn x -> -1*x end)
	  pop_diff = decrease ++ increase
	  {Enum.sum(populations), n_village, n_active_village, population_increase, population_decrease, populations, pop_diff}
      end
    end
  end
  ...
  def random_result() do
    let [
      player_id <- bitstring(),
      {population_total, n_village, n_active_village, population_increase, population_decrease, populations, pop_diff} <- create_pops(),
      ...
      villages = [races, populations, pop_diff, date_diffs]
      |> Enum.zip_reduce([], fn elems, acc -> [List.to_tuple(elems) | acc] end)
      input = {{player_id, date}, villages}
      output = {player_id, date, hd(date_diffs), n_village, n_active_village, population_total, population_increase, population_decrease, n_races}
      {input, output}
    end
  end
...
##################
    ...
    #   date = elem(output, 1)
    #   result = Medusa.PlayerHistoric.create_player_attrs(input)
