defmodule MedusaPipeline do
  ...
  """
  @type input ::
          {TTypes.player_id(), TTypes.village_id(), TTypes.date(), TTypes.race(),
           TTypes.population()}
  @type output :: %{
          player: TTypes.player_id(),
          date: TTypes.date(),
  ...
  def apply(input) do
    ...
    |> Enum.group_by(
      ...
      fn {_, _, _, race, population, pop_diff, date_diff} ->
        {race, population, pop_diff, date_diff}
      end
    )
  ...
  @spec assign_ids(
          {{TTypes.player_id(), TTypes.village_id()},
           [
             {TTypes.date(), TTypes.race(), TTypes.population(), TTypes.population_diff(),
              TTypes.date_diff()}
           ]}
        ) ::
          [
            {TTypes.player_id(), TTypes.village_id(), TTypes.date(), TTypes.race(),
             TTypes.population(), TTypes.population_diff(), TTypes.date_diff()}
          ]
  defp assign_ids({ids, village_log}),
    do: Enum.map(village_log, fn vlog -> assing_ids(ids, vlog) end)
  ...
  defp make_output_map(
         {player_id, date, date_diff, n_village, n_active_village, population,
