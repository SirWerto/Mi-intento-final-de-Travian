defmodule MedusaPipeline_VillageHistoric do
  use ExUnit.Case
  ...
  def make_village_logs(init_date, init_pop, race, date_increments, pop_increments) do
    ...
    Enum.zip_reduce([dates, races, pops], [], fn elements, acc ->
  ...
  def info_consecutive() do
    let [
      ...
      {date_increments, pop_increments} <- increments_consecutives()
    ] do
      start_date = ~D[2019-09-05]

      village_logs =
        make_village_logs(start_date, start_population, race, date_increments, pop_increments)

      {player_id, village_id, race, start_population, pop_increments, start_date, date_increments,
       village_logs}
    end
  end
  ...
  def info_random() do
    let [
      ...
      race <- random_race(),
      start_population <- pos_integer(),
      {date_increments, pop_increments} <- increments_random()
    ] do
      start_date = ~D[2019-09-05]
  ...
  property "create_village_attr maintains player_id and village_id" do
    forall info <- info_random() do
      ...
      {{player_id_2, village_id_2}, _results} =
        MedusaPipeline.VillageHistoric.create_village_attrs(
          {{player_id, village_id}, village_logs}
        )
  ...
  property "create_village_attr race don't change over the pipeline" do
    forall info <- info_random() do
      ...
      {{_player_id, _village_id}, results} =
        MedusaPipeline.VillageHistoric.create_village_attrs(
          {{player_id, village_id}, village_logs}
        )

      assert(
        Enum.all?(results, fn x -> elem(x, 1) === race end),
        "race changes over the pipeline"
      )
    end
  end
  ...
  property "create_village_attr date_diff === 1 when consecutive days" do
    forall info <- info_consecutive() do
      ...
      assert(
        Enum.all?(results, fn x -> elem(x, 4) === 1 end),
        "date diff != 1 when consecutive days"
      )
    end
  end
  ...
  property "pop diff computation test" do
    forall info <- info_random() do
      ...
      bool =
        Enum.map(results, fn x -> elem(x, 3) end)
  ...
  property "date diff computation test" do
    forall info <- info_random() do
      ...
      bool =
        Enum.map(results, fn x -> elem(x, 4) end)
