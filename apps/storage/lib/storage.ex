defmodule Storage do
  ...
  @spec store(root_folder :: String.t(), identifier :: dest_identifier(), flow_options :: flow_options(), content :: binary(), date :: Date.t()) :: :ok | {:error, any()}
  def store(root_folder, identifier, {flow_name, flow_extension}, content, date \\ Date.utc_today()) do
  ...
  @spec open(root_folder :: String.t(), identifier :: dest_identifier(), flow_options :: flow_options(), open_options) :: {:ok, {Date.t(), binary()}} | {:ok, [{Date.t(), binary()}]} | {:error, any()}
  ...
  def open(root_folder, identifier, flow_options, {start_date, end_date, :consecutive}) do
      case Date.compare(start_date, end_date) do
  	:gt -> {:error, "end_date earlier than start_date"}
  	:eq -> open(root_folder, identifier, flow_options, start_date)
  	:lt ->
  	  result = gen_date_range!(start_date, end_date)
  ...
  defp open_rec([date | dates], contents, root_folder, identifier, flow_options) do
    case open(root_folder, identifier, flow_options, date) do 
      ...
      {:ok, content} -> open_rec(dates, contents ++ [content], root_folder, identifier, flow_options)
    end
  end

  @spec gen_dir_path(root_folder :: binary(), identifier :: dest_identifier(), flow_name :: flow_name()) :: binary()
  defp gen_dir_path(root_folder, :global, flow_name), do: "#{root_folder}/global/#{flow_name}"
  defp gen_dir_path(root_folder, server_id, flow_name), do: "#{root_folder}/servers/#{format_server_id(server_id)}/#{flow_name}"
  @spec gen_filename(dir_path :: binary(), date :: Date.t(), flow_extension :: flow_extension()) :: binary()
  defp gen_filename(dir_path, date, flow_extension), do: "#{dir_path}/date_#{Date.to_iso8601(date, :basic)}#{flow_extension}"
